...
import { isKnownFontValue } from "./fontRegistry";

const ALLOWED_TABLE_HEADER_ALIGNMENTS = new Set(["left", "center", "right"]);
...
function mergeElementStyle(rawStyle) {
  ...
  if (!isKnownFontValue(merged.fontFamily)) {
...
export function mergeStyleConfig(rawConfig) {
  ...
  merged.themeRoundedCorners = merged.themeRoundedCorners === true;
  merged.tableHeaderAlign = ALLOWED_TABLE_HEADER_ALIGNMENTS.has(merged.tableHeaderAlign)
    ? merged.tableHeaderAlign
    : defaults.tableHeaderAlign;
  merged.tableFullWidth = Boolean(merged.tableFullWidth);
