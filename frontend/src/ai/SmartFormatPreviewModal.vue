...
function syncPreviewScroll(source, target) {
  ...
  target.scrollTop = sourceMaxTop > 0 ? (source.scrollTop / sourceMaxTop) * targetMaxTop : 0;
...
<template>
  <div v-if="props.visible" class="format-preview-backdrop" @mousedown.self="emit('close')">
    <section class="format-preview-modal" role="dialog" aria-modal="true">
      <header class="format-preview-header">
        <div>
          <span class="eyebrow">SMART FORMAT PREVIEW</span>
          <h2>确认是否使用智能排版结果</h2>
          <p>左右两侧均为渲染后的阅读效果，方便直接比较层级、留白、列表、表格和图表。确认后才会写回文档。</p>
        </div>
